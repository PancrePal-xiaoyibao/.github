<!--
title: 🌐 Pillar 7 · 跨癌种/罕见病方法学复制 · Methodology Replication
labels: pinned, pillar-7, vision, help-wanted, replication, cross-disease
parent: 00-billboard
-->

> 🔙 **返回** [🌟 社区愿景 BillBoard 顶层 issue #24](https://github.com/PancrePal-xiaoyibao/.github/issues/24)

# 🌐 Pillar 7 · 跨癌种 / 罕见病方法学复制

> **把 3 年积累的 ACT 框架（AI-Community-Toolkits）复制到每一个需要它的病种社区，让"微光成炬"真正成炬。**

## 🌠 为什么这件事重要

- **中国有 1500+ 罕见病**：绝大多数没有专业社区，患者和家属散落在各处。
- **癌种间的经验不互通**：小胰宝踩过的坑，小肺宝还在重踩；小萌宝解决的问题，小铃铛还在摸索。
- **ACT 框架已成型**：3 年迭代总结出**20% 技术 + 80% 人文**的开源公益 AI 社区框架 —— 从组织架构、蓝马甲机制、CMC 治理、微光成炬、到八波招募，每一环都可复制。

**我们的目标**：让**任何一位有热情的病种主理人**，都能在 **3-6 个月内** 基于小 X 宝框架建立起自己的病种 AI 社区，覆盖从 AI 助手上线、志愿者招募、蓝马甲试点、心理小组到医院合作的全部流程。

## 🎯 里程碑清单（M7）

| # | 里程碑 | 难度 | 预估时长 | 状态 |
|:---:|---|:---:|:---:|:---:|
| **M7.1** | **ACT 框架白皮书**：把 3 年经验固化成可复制的文档 + 视频 + Checklist | 🟡 中 | 3-4 周 | 🟡 招募中 |
| **M7.2** | **"新病种主理人"onboarding SOP**：从"我想做"到"上线"的 90 天路线图 | 🟢 中 | 2 周 | 🔵 待认领 |
| **M7.3** | **第 8 波秋季招募** 深化：临床/RAG/病友主理人/运营/微光成炬 5 大角色扩招 | 🟡 中 | 持续 | 🟡 招募中 |
| **M7.4** | **DKU 学生实践项目扩展**：见 [DKU-Programs-Hub](https://github.com/PancrePal-xiaoyibao/DKU-Programs-Hub) | 🟢 中 | 每学期 | 🔵 待认领 |
| **M7.5** | **医院/科室合作模板**：把小铃铛 / 胰小助 / 胰管家的合作协议、部署流程、运营 SOP 开源出来 | 🟢 中 | 2 周 | 🔵 待认领 |
| **M7.6** | **微光成炬企业伙伴 onboarding**：Token 捐赠 / 算力捐赠 / 联合品牌的标准流程 | 🟢 低 | 1 周 | 🟢 好上手 |
| **M7.7** | **社区志愿者积分平台**：见 2025-5 志愿者积分注册项目 | 🔴 高 | 4-6 周 | 🔵 招募主理人 |
| **M7.8** | **国际化 & 出海**：pbs 无国界小组 + pancan.org 合作 + 海外社交媒体矩阵 | 🔴 高 | 长期 | 🔵 招募国际 SIG |

## 🚪 不同人群怎么参与

<table>
<tr>
<td width="25%" valign="top">

### 💪 想当主理人的你
- 有热情、有病种痛点体感
- 走 **M7.2** 90 天路线图
- 会被匹配导师 & 资源

</td>
<td width="25%" valign="top">

### 🎓 学生 / 研究生
- **M7.4** DKU 项目
- 学分 + 实践 + 社区贡献三合一
- 联系 CMC-Lisa / zhuangbiaowei

</td>
<td width="25%" valign="top">

### 🏥 医院 / 科室
- **M7.5** 合作模板
- 从科室内部 AI 助手起步
- 可申请微光成炬资源支持

</td>
<td width="25%" valign="top">

### 🌍 海外华人
- **M7.8** 国际化方向
- 帮忙翻译、连接海外资源
- 参与 pbs 无国界小组

</td>
</tr>
</table>

## 🔗 相关资源

- **主仓库**：
  - [hackathon2026](https://github.com/PancrePal-xiaoyibao/hackathon2026) · ★ **2026 社区黑客松**，M7.3 第 8 波招募与跨病种复制的旗舰活动
  - [DKU-Programs-Hub](https://github.com/PancrePal-xiaoyibao/DKU-Programs-Hub) · [DKU-Programs-Wiki](https://github.com/PancrePal-xiaoyibao/DKU-Programs-Wiki)
  - [MedGemma3 测试](https://github.com/PancrePal-xiaoyibao/MedGemma3_test_xyb)
- **参考项目**：pancan.org SPARK · Rare Disease Alliance · Linux Foundation Public Health
- **推荐 skill**：`/executive-consultant` `/nsfc-proposal-advisor`（VitaForge · 战略维度）
- **社区文档**：
  - [了解社区](https://hi.xiao-x-bao.com.cn) · [志愿者责任](https://faq.xiao-x-bao.com.cn) · [加入我们](https://iamin.xiao-x-bao.com.cn)
  - [社区任务·梦想项目](https://task.xiaoyibao.com.cn)

## 📈 成功指标

- ✅ **10+ 病种社区** 基于 ACT 框架上线（当前：小胰宝 / 小肺宝 / 小萌宝 / 小胃宝 / 小妍宝 / 小粉宝 / 小铃铛 = 7）
- ✅ **20+ 医院/科室** 合作案例
- ✅ **第 8 波招募** 累计新增志愿者 > 100

## 👥 现有 mentor / 导师

- **@samqin123**（ACT 框架作者）
- **@RichardLin**（开源社老师）
- **@庄老师**（zhuangbiaowei · CMC）
- **@Lisa**（CMC / DKU 项目）
- 招募中：国际化 SIG 组长

---

**如何认领**：在本 issue 下评论 `我想接手 M7.x，方向 [主理人/学生/医院/海外]，预计 X 天内提 PR / 交付物`。
